...
#    print('raw: ' + str(entry))
    if (entry[0] == 0xE5):
        ...
        blocksUsed = '  '
        fileBaseName = str(trim(stripBit8(entry[1:9])))
        fileExt = str(trim(stripBit8(entry[9:12])))
        fileName = fileBaseName + '.' + fileExt
        for block in range(0x10,0x20):
            if (entry[block] != 0):
                blocksUsed = blocksUsed + hex(block) + '/' + str(hex(entry[block])) + ' '
                fileBlocks.keys()
                addTo(fileBlocks, fileName, entry[block])
                
        retStr = str(entry[0]) + ':' + fileBaseName + '.' + fileExt + ' ' + extend + ' ' + ' ' + str(blocksUsed)
        # 
        return retStr
...
def addTo(fbDict, key, value):
    if (fbDict.get(key)):
        fbDict[key].append(int(value))
    else:
        fbKeyList = list()
        fbKeyList.append(int(value))
        fbDict[key] = fbKeyList
        
dirBlocks = int(dirEntCount / 4)
blockStart = dirBlocks * sectorSize
print('  blockStart: ' + hex(blockStart) + ', typeByte: ' + hex(typeByte))
if (firstSize == 0x20):
    if (typeByte == 0x37):
        blockStart += 0x4000
    elif (typeByte == 0x50):
...
print('Block start offset: ' + str(blockStart))
...
def saveFile(fileName, blockList):
    cpmFile = open(fileName,"wb")
    blockCount = len(blockList)
    file.seek(0)
    for blk in blockList:
        blockAddr = (blk * blockSize + blockStart) & 0x7FFF
        print(' blk: ' + hex(blk) + ',  addr: ' + hex(blockAddr))
        file.seek(blockAddr)
        data = file.read(1024)
...
for dirEnt in fileBlocks:
    blks = ''
    print('File: ' + dirEnt)
    for blk in fileBlocks[dirEnt]:
        blks += hex((blk * blockStart) & 0x7FFF) + ', '
    print(' blks: ' + blks)
    saveFile(dirEnt, fileBlocks[dirEnt])
